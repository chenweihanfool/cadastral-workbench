...
with open('clipped_merged_parcels.pkl', 'rb') as f:
    d = pickle.load(f)
...
# ── COA：只對「篩選後實際用到」的點重新編號（比照 clip_task/clip.py 的做法，
# 不像 export_final.py 那樣把所有 11 分幅的全部點都編進去）───────────────
new_pid = {}
coa_lines = []
next_pid = 1
for mkey in merged_parcels:
    for p in merged_parcels[mkey]['ring']:
        if p not in new_pid:
            y, x = coordlookup[p]
            new_pid[p] = next_pid
...
coa_text = '\r\n'.join([f'KC0336{n_points:5d}  500  {n_points:5d}'] + coa_lines) + '\r\n'
bnp_text = '\r\n'.join([f'KC0336{len(bnp_lines):5d}'] + bnp_lines) + '\r\n'
par_text = '\r\n'.join([f'KC0336{n_parcels_out:5d}  0 0 09006'] + par_lines) + '\r\n'

with open('KC0336_clipped_joined.COA', 'w', encoding='big5', newline='') as f:
    f.write(coa_text)
with open('KC0336_clipped_joined.BNP', 'w', encoding='big5', newline='') as f:
    f.write(bnp_text)
with open('KC0336_clipped_joined.PAR', 'w', encoding='big5', newline='') as f:
    f.write(par_text)

n_spliced = sum(1 for v in merged_parcels.values() if v.get('spliced'))
print(f'輸出完成：COA {n_points} 點、BNP/PAR {n_parcels_out} 筆地號（其中真正接圖成功 {n_spliced} 筆、撞號調整 {len(renumbered)} 筆）')
